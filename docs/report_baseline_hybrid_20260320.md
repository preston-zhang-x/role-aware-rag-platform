...
| 8 | IF-007 の HTTP メソッドは何ですか？ | admin | admin | HIT | 5 | - | - |
...
| 20 | chunk_overlap は何ですか？ | admin | admin | MISS | 5 | keyword_mismatch | - |
...
- **実際の回答**: 仕様書のタイトルは「Role Aware RAG Platform」です。（出典: docs/role_aware_rag_platform_spec.xlsx）
...
- **先頭 Source**: docs/role_aware_rag_platform_spec.xlsx
- **実際の回答**: ITM-SCR003-002は、現在ログイン中ユーザーのroleを表示します。出典は【参考1】です。
...
- **不足キーワード**: token_type
- **先頭 Source**: docs/role_aware_rag_platform_spec.xlsx
- **実際の回答**: IF-001 の response に含まれるトークン種別の項目名は「Token」です。（出典: 参考1）
...
- **先頭 Source**: docs/role_aware_rag_platform_spec.xlsx
- **実際の回答**: documents.updated_at の自動/更新は「func.now()」です。これは、更新日時が自動的に現在の日時に設定されることを意味します。（出典: 参考1）
...
- **不足キーワード**: 100
- **先頭 Source**: docs/role_aware_rag_platform_spec.xlsx
- **実際の回答**: chunk_overlapは、ベクトルストア仕様において、チャンクの重複部分のサイズを示す設定です。具体的には、チャンクを分割する際に、どれだけのサイズの部分が重複しているかを指定します。この設定により、情報の一貫性を保ちながら、データを効率的に処理することが可能になります。（出典: 参考2）
...
---
*Generated by `script/run_eval.py` at 2026-03-20 00:37:10*