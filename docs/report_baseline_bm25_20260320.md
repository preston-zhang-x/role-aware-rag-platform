...
|----|----------|----------------|-------------|-----|---------|-------------|-------|
| 1 | 仕様書のタイトルは何ですか？ | admin | admin | MISS | 5 | keyword_mismatch | - |
| 2 | 作成対象は何ですか？ | admin | admin | MISS | 5 | not_found_answer | - |
| 3 | FN-007 の機能名は何ですか？ | admin | admin | HIT | 5 | - | - |
| 4 | FN-010 のカテゴリは何ですか？ | admin | admin | HIT | 5 | - | - |
...
| 11 | IF-004 の query parameter `limi... | admin | admin | MISS | 5 | not_found_answer | - |
| 12 | IF-004 の `limit` の上限は何ですか？ | admin | admin | HIT | 5 | - | - |
| 13 | IF-005 の path parameter 名は何ですか... | admin | admin | HIT | 5 | - | - |
| 14 | IF-007 の成功時ステータスは何ですか？ | admin | admin | MISS | 5 | not_found_answer | - |
| 15 | IF-008 の request body フィールド名は何... | admin | admin | MISS | 5 | not_found_answer | - |
...
| 20 | chunk_overlap は何ですか？ | admin | admin | HIT | 5 | - | - |
...
| 26 | IF-003 の成功時ステータスは何ですか？ | manager | admin | MISS | 5 | keyword_mismatch | - |
| 27 | IF-006 の認可は何ですか？ | manager | admin | HIT | 5 | - | - |
| 28 | IF-009 の名称は何ですか？ | staff | admin | MISS | 5 | not_found_answer | - |
...
| 35 | ACCESS_TOKEN_EXPIRE_MINUTES のコ... | staff | admin | HIT | 5 | - | - |
...
| 40 | RAG の response sources に含まれる項目... | admin | admin | HIT | 5 | - | - |
...
- **不足キーワード**: Role Aware RAG Platform 現状仕様書
- **先頭 Source**: docs/role_aware_rag_platform_spec.xlsx
- **実際の回答**: 仕様書のタイトルは「Role Aware RAG Platform」です。（出典: 参考4）
...
- **先頭 Source**: docs/role_aware_rag_platform_spec.xlsx
- **実際の回答**: ITM-SCR003-002は、現在ログイン中ユーザーのroleを表示します。（出典: 参考1）
...
- **不足キーワード**: token_type
- **先頭 Source**: docs/role_aware_rag_platform_spec.xlsx
- **実際の回答**: IF-001 の response に含まれるトークン種別の項目名は「Token」です。（出典: 参考5）

### Q11: IF-004 の query parameter `limit` の既定値は何ですか？
- **Requested Role**: admin
- **Executed As**: admin
- **Miss Reason**: not_found_answer
- **期待キーワード**: 10
...
### Q15: IF-008 の request body フィールド名は何ですか？
- **Requested Role**: admin
- **Executed As**: admin
- **Miss Reason**: not_found_answer
- **期待キーワード**: question
...
- **先頭 Source**: docs/role_aware_rag_platform_spec.xlsx
- **実際の回答**: IF-010 の異常時ステータスは「503 degraded」です。（出典: 参考3）
...
- **先頭 Source**: docs/role_aware_rag_platform_spec.xlsx
- **実際の回答**: documents.updated_at の自動/更新は「func.now()」であり、これは更新日時を現在の日時に設定することを意味します。（出典: 【参考1】）
...
- **不足キーワード**: 201, Created
- **先頭 Source**: docs/role_aware_rag_platform_spec.xlsx
- **実際の回答**: IF-003 の成功時ステータスは、200 OK です。（出典: docs/role_aware_rag_platform_spec.xlsx）
...
---
*Generated by `script/run_eval.py` at 2026-03-20 00:31:41*